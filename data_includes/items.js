...
//Other params:
var train_block_num = 5; //Should be 5 in real exp.
var test_block_num = 5; //Should be 5 in real exp.
...
for (var block_num = 0; block_num < train_block_num; block_num++){
    ...
    if (test_run){
        ...
        block_length = block_template.length;   
    }
    total_trials += block_length;
                
    for (var trial = 0; trial < block_length; trial ++){
        ...
        //Create intro page for the block:
        if (trial == 0){
            train_items.push(["block_intro_"+block_num, "Message", {html:"<h1>Training Block "+(block_num+1)+" Start</h1><p>You are about to begin a training block."+
                                                                         " In this block, you’ll hear examples of nouns in the alien language that are combined with the "+
                                                                         " suffix that corresponds to the meaning below:</p>"+urImage}]);
            train_names.push("block_intro_"+block_num); 
            
            //Randomize which side the correct answer happens on:
            var word_options = ["A", "B"];
            shuffle(word_options);
            var correct_word =  word_options[0];
        
            //Set up the stimulus pairs:
            wrong_ur_file = ur_dir.concat(suff).concat("_").concat(suff2vowel[this_suff]).concat(this_suff).concat("_wrong.wav");
            if (correct_word == "A"){
                var audio_a = "<audio style='visibility:hidden;' id='a_player' controls onended='audioEndPostA()'><source src='".concat(ur_file).concat("' type='audio/wav'></audio>");
                var audio_b = "<audio style='visibility:hidden;' id='b_player' controls onended='audioEndPostB()'><source src='".concat(wrong_ur_file).concat("' type='audio/wav'></audio>");          
            }
            else {
                var audio_a = "<audio style='visibility:hidden;' id='a_player' controls onended='audioEndPostA()'><source src='".concat(wrong_ur_file).concat("' type='audio/wav'></audio>");
            ...
            var choices_html = silence_one.concat(silence_two).concat(audio_a).concat(audio_b).concat(choice_functions).concat("<table align='center'><tr><td align='center'><div align='center'><i>How is the suffix that corresponds to the picture above pronounced when it’s not combined with a noun?</i></div></td></tr></table><table align='center'><tr><td><font color='white'>__</font><span id='option_A'>A</span></td><td><p style='visibility:hidden;'>__</p></td><td><font color='white'>__</font><span id='option_B'>B</div></td></tr></table>");
            train_items.push(["block_SuffTestA_"+block_num, "my_comicCaption", {s:"", q:choices_html,
                                                                         html:"http://people.umass.edu/bprickett/Opacity_Denial/outlined_square.jpg",
                                                                         mean:this_meaning+"noEq", hasCorrect:correct_word, as:["A", "B"]}]);
            train_names.push("block_SuffTestA_"+block_num);
            train_names.push("comp_feedback");
        }
    ...
    var choices_html = silence_one.concat(silence_two).concat(audio_a).concat(audio_b).concat(choice_functions).concat("<table align='center'><tr><td align='center'><div align='center'><i>How is the suffix that corresponds to the picture above pronounced when it’s not combined with a noun?</i></div></td></tr></table><table align='center'><tr><td><font color='white'>__</font><span id='option_A'>A</span></td><td><p style='visibility:hidden;'>__</p></td><td><font color='white'>__</font><span id='option_B'>B</div></td></tr></table>");
    train_items.push(["block_SuffTestB_"+block_num, "my_comicCaption", {s:"", q:choices_html,
                                                                         html:"http://people.umass.edu/bprickett/Opacity_Denial/outlined_square.jpg",
                                                                         mean:this_meaning+"noEq", hasCorrect:correct_word, as:["A", "B"]}]);
    train_names.push("block_SuffTestB_"+block_num);
    train_names.push("comp_feedback");
...
console.log("Training trials", total_trials);
//**************************************
...
for (var block_num = 0; block_num < test_block_num; block_num++){
    ...
    //Create suffix test before the first and third blocks:
    if (block_num == 0 || block_num == 2){
        shuffle(block_suffs);    
        for (var suff_num = 0; suff_num < block_suffs.length; suff_num++) {   
            var this_suff =  block_suffs[suff_num];
            var this_meaning = suffix_to_meaning[this_suff];
            //Randomize which side the correct answer happens on:
            var word_options = ["A", "B"];
            shuffle(word_options);
            ...
            var choices_html = silence_one.concat(silence_two).concat(audio_a).concat(audio_b).concat(choice_functions).concat("<table align='center'><tr><td align='center'><div align='center'><i>How is the suffix that corresponds to the picture above pronounced when it’s not combined with a noun?</i></div></td></tr></table><table align='center'><tr><td><font color='white'>__</font><span id='option_A'>A</span></td><td><p style='visibility:hidden;'>__</p></td><td><font color='white'>__</font><span id='option_B'>B</div></td></tr></table>");
            test_items.push(["TestPhase_SuffTest_"+block_num+"_"+suff_num, "my_comicCaption", {s:"", q:choices_html,
                                                                              html:"http://people.umass.edu/bprickett/Opacity_Denial/outlined_square.jpg",
                                                                              mean:this_meaning+"noEq", hasCorrect:correct_word, as:["A", "B"]}]);
            test_names.push("TestPhase_SuffTest_"+block_num+"_"+suff_num);
            test_names.push("comp_feedback");
        }
    }
    
    for (var tt = 0; tt < block_length; tt++){
        //Find the stem and corresponding picture for this trial:
        pic_file = pic_names.pop();
...
shuffle(block_suffs);    
for (var suff_num = 0; suff_num < block_suffs.length; suff_num++) {   
    this_suff =  block_suffs[suff_num];
    var this_meaning = suffix_to_meaning[this_suff];
...
//Set up the stimulus pairs:
if (correct_word == "A"){
           var audio_a = "<audio style='visibility:hidden;' id='a_player' controls onended='audioEndPostA()'><source src='".concat(audio_dir+"s.wav").concat("' type='audio/wav'></audio>");
           var audio_b = "<audio style='visibility:hidden;' id='b_player' controls onended='audioEndPostB()'><source src='".concat(audio_dir+"z.wav").concat("' type='audio/wav'></audio>");          
}
else {
           var audio_a = "<audio style='visibility:hidden;' id='a_player' controls onended='audioEndPostA()'><source src='".concat(audio_dir+"z.wav").concat("' type='audio/wav'></audio>");
           var audio_b = "<audio style='visibility:hidden;' id='b_player' controls onended='audioEndPostB()'><source src='".concat(audio_dir+"s.wav").concat("' type='audio/wav'></audio>");  
}
            
var choices_html = silence_one.concat(silence_two).concat(audio_a).concat(audio_b).concat(choice_functions).concat("<table align='center'><tr><td align='center'><div align='center'><i>How is the suffix that corresponds to the picture above pronounced when it’s not combined with a noun?</i></div></td></tr></table><table align='center'><tr><td><font color='white'>__</font><span id='option_A'>A</span></td><td><p style='visibility:hidden;'>__</p></td><td><font color='white'>__</font><span id='option_B'>B</div></td></tr></table>");
practice_items.push(["practice_SuffTestA", "my_comicCaption", {s:"", q:choices_html,
                                                                         html:"http://people.umass.edu/bprickett/Opacity_Denial/outlined_square.jpg",
                                                                         mean:"PracticenoEq", hasCorrect:correct_word, as:["A", "B"]}]);
practice_names.push("practice_SuffTestA");
practice_names.push("comp_feedback");
...
var choices_html = silence_one.concat(silence_two).concat(audio_a).concat(audio_b).concat(choice_functions).concat("<table align='center'><tr><td align='center'><div align='center'><i>How is the suffix that corresponds to the picture above pronounced when it’s not combined with a noun?</i></div></td></tr></table><table align='center'><tr><td><font color='white'>__</font><span id='option_A'>A</span></td><td><p style='visibility:hidden;'>__</p></td><td><font color='white'>__</font><span id='option_B'>B</div></td></tr></table>");
practice_items.push(["practice_SuffTestB", "my_comicCaption", {s:"", q:choices_html,
                                                                         html:"http://people.umass.edu/bprickett/Opacity_Denial/outlined_square.jpg",
                                                                         mean:"PracticenoEq", hasCorrect:correct_word, as:["A", "B"]}]);
practice_names.push("practice_SuffTestB");
practice_names.push("comp_feedback");        
...
items.push([
                   ...
                   {html:  
                          ...
                          +"<p><span style='font-weight:bold;'>In this experiment, you’ll be learning how to pronounce different combinations of words and suffixes.</span> But first, we’re going to do some practice trials in English.</p>"
                    }
            ]);        
 ...
 items.push([
  ...
  items.push([
                  ...
                  {html:
                          ...
                          +"<img style='height: 400px; width:350px' src='https://people.umass.edu/bprickett/Opacity_Denial/equals cats example.png'>"
                   }
              ]);
...
items.push([
                  ...
                  "Message",
                  {html:
                          "<h1>Suffix Meaning Tests</h1>"
                          +"<p>While you don’t need to memorize the individual nouns, we do want to make sure you learn the alien suffixes for each direction.</p>"
                          +"<ul><li>In training, there will be some trials to make sure you’re learning how the suffixes are pronounced.</li>"
                          +"<li>You’ll be asked to choose between two pronunciations of the suffix.</li></ul>"
                          +"<p>We’ will also have these trials in Testing to check if you have learned them.</p>"
                   }
              ]);
...
items.push([
                  ...
                  {html:
                          ...
                          +"<p>Also pay attention to how the suffix for each direction is pronounced, since you’ll be tested on this throughout the experiment.</p>"
...
items.push([
                   ...
                   {html: 
                         ...
                         " Remember to pay attention to how both the words and suffixes are pronounced when they are combined.</p>"
...
items = items.concat([    
               ...
               [
                   ...
                   {
                       transfer: 15000,
                       html: "<div><p>All done! To receive compensation, click <a href='https://app.prolific.co/submissions/complete?cc=726F466F'>here</a>.</p></div>"
                   }
               ],
               [
                   "preload_audio",
...
var all_trials = [].concat([  
                    ...
                    "Instr3", "Instr3.5","compQ3", "comp_feedback", "Instr3.75", "Instr4", "compQ4", "comp_feedback", "Instr5"
                  ]);
...
var defaults = [
    ...
    "my_comicCaption", { //Options for ComicCaption items
        //as: [["y","Yes"], ["n","No"]],
        as: ["Yes", "No"],
        presentHorizontally: true,
        instructions: "",
